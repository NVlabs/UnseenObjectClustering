...
class Tabletop_Object_Demo(object):
    ...
    def run_on_opencv_image(self, image, rgb_image):
        ...
        if self.show_mask_map_nice:
            ...
            return self.create_mask_map(result, top_predictions), boxes
    ...
    def create_mask_map(self, image, predictions):
        """ 
        Create a mask map similar to my other networks. 
        Produce a mask with values {0, ..., K-1}, 
            where 0 is background, 1 is table, 2+ are objects
        ...
        masks = predictions.get_field("mask").numpy() # Shape: [num_instances x 1 x H x W]. Values in {0, 1}
        labels = predictions.get_field("labels") # Shape: [num_instances]. Values in {1}
        # Note: masks/labels are in descending score order

        obj_id = 1 # start at 1
        result = np.zeros((H,W))
        for idx in range(len(labels))[::-1]: # from lowest score to highest
            mask = masks[idx, 0] # Shape: [H x W]
            if labels[idx] == 2:
                result_label = obj_id
                result[mask == 1] = result_label
                obj_id += 1
