...
import torch
import unseen_clustering_cuda


class HardLabelFunction(Function):
    @staticmethod
    def forward(ctx, prob, label, rand, threshold, sample_percentage):
        outputs = unseen_clustering_cuda.hard_label_forward(threshold, sample_percentage, prob, label, rand)
        top_data = outputs[0]
        return top_data

    @staticmethod
    def backward(ctx, top_diff):
        outputs = unseen_clustering_cuda.hard_label_backward(top_diff)
