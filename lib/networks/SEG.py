...
__all__ = [
    'seg_vgg', 'seg_unet', 'seg_vgg_embedding', 'seg_unet_embedding', 'seg_resnet34_8s_embedding_early',
...
class SEGNET(nn.Module):
    ...
    def __init__(self, init_weights=True, batch_norm=False, in_channels=3,
        ...
        if network_name == 'vgg':
            ...
            if not embedding:
                ...
                # A 1x1 conv layer that goes from embedded features to logits for foreground (UOIS)
                self.fg_module = nn.Conv2d(num_units, 1, kernel_size=1, stride=1, padding=0, bias=False)
        else:
            if embedding:
    ...
    def forward(self, img, label, depth=None):
        ...
        if self.embedding:
            ...
            features = self.decoder(en)
            logits = self.fg_module(features)[:,0,:,:]
...
def seg_resnet34_8s_embedding_early(num_classes=2, num_units=64, data=None):
    model = SEGNET(in_channels=6, network_name='Resnet34_8s', embedding=True, num_units=num_units)
    update_model(model, data)
    return model
